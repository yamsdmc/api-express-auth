...
import { createValidProduct, createValidProductListing } from "@application/use-cases/product-listing/factories/productListing.factory";
...
describe("InMemoryProductListingRepository", () => {
    ...
    describe("update", () => {
        it("should throw error for non-existent listing", async () => {
            await expect(
                repository.update("non-existent", createValidProductListing({
        ...
        it("should update product data", async () => {
            const created = await repository.create(mockListing);
            console.log(created)
            const updated = await repository.update(created.id!, createValidProductListing({
                product: createValidProduct({price: 600})
            }));
    ...
    describe('findAll', () => {
        const createListing = (product: Partial<ProductEntity>) => createValidProductListing({product: createValidProduct(product)})

        const mockListings = [
            createListing({ title: 'macbook air', category: ProductCategory.ELECTRONICS, price: 400}),
            createListing({ title: 'macbook pro', category: ProductCategory.CLOTHING, price: 1000}),
        ];
